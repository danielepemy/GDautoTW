...
def run_git_checked(args: Sequence[str], repo_root: Path, error_prefix: str) -> subprocess.CompletedProcess:
...
def ensure_branch(repo_root: Path, log: Callable[[str], None]) -> str:
    ...
    log(f"Preparing branch {branch}")
    run_git_checked(["fetch", "origin"], repo_root, "git fetch failed")
    ...
    run_git_checked(["checkout", "-b", branch], repo_root, "git checkout -b failed")
    return branch
...
def commit_and_push(
    ...
    upstream = run_git(["rev-parse", "--abbrev-ref", "--symbolic-full-name", "@{u}"], repo_root)
...
class MainWindow(QtWidgets.QWidget):
    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("GD_autoTW_v02")
        self.resize(720, 540)
